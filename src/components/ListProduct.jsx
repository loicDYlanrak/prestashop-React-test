...
import { useFetchAllProduits, fetchPrestashop } from "../hooks/useFetchPrestashop.js";
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (data && productID) {
                ...
                const combinationsData = response.data?.product?.associations?.combinations?.combination;
                // console.log("Combinations data:", combinationsData);
                const combinationsId = combinationsData?.map((combo) => combo.id?.["#cdata"]);
                ...
                const combinationsWithDetails = combinationsData?.map((combo) => {
                    const comboDetail = combinationsDetails.find((detail) => detail.combination.id?.["#cdata"] === combo.id?.["#cdata"]);
                    // console.log(`Combo detail for combination ID ${combo.id?.["#cdata"]}:`, comboDetail);
                    const optionValueId = comboDetail?.combination?.associations?.product_option_values?.product_option_value?.id?.["#cdata"] ;
                    const productId = comboDetail?.combination?.id_product?.["#cdata"];
                    const price = comboDetail?.combination?.price?.["#cdata"];
                    // console.log(`Option values for combination ID ${combo.id?.["#cdata"]}:`, optionValueId);
                    return {
                        id: combo.id?.["#cdata"],
                ...
                const optionValueIds = combinationsWithDetails.map((combo) => combo.optionValueId);
                ...
                const optionsProductValues = optionValuesDetails.map((optionValue) => {
                    ...
                    return {
                        ...
                        groupId: optionValue.product_option_value.id_attribute_group?.["#cdata"],
                    }
                });
                ...
                const optionsProductsDetails = await Promise.all(
                    optionsProductValues.map(async (optionValue) => {
                        const optionResponse = await fetchPrestashop(`product_options/${optionValue.groupId}`);
                ...
                const optionsWithValues = optionsProductValues.map((optionValue) => {
                    const optionDetail = optionsDetails.find((option) => option.id === optionValue.groupId);
                    return {    
                        ...optionValue,
                ...
                const stocksData = response.data?.product?.associations?.stock_availables?.stock_available;
                ...
                stocks.map((stock) => {
                    const stockDetail = stocksDetails.find((detail) => detail.stock_available.id?.["#cdata"] === stock.id);
                    stock.quantity = stockDetail?.stock_available?.quantity?.["#cdata"] || stock.quantity;
                });
                ...
                const stocksOptions = StocksCombinations.map((combo) => {
                    const optionValue = optionsWithValues.find((option) => option.id === combo.optionValueId);
                    return {
                        ...combo,
